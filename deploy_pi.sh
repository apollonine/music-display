#!/bin/bash
# Raspberry Pi deployment script - run as pi user
...
if [ "$USER" != "pi" ]; then
    echo "❌ This script must be run as 'pi' user"
    echo "   Run: ssh pi@raspberrypi.local"
    echo "   Then: bash <(curl -sSL https://raw.githubusercontent.com/apollonine/music-display/main/deploy_pi.sh)"
    exit 1
fi

# Update system (will prompt for password)
